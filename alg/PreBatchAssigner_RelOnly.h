...
#ifndef DISTRESCAL_PREBATCHASSIGNER_RELONLY_H
#define DISTRESCAL_PREBATCHASSIGNER_RELONLY_H
...
public:
    ...
    explicit PreBatch_assigner_rel(int n, const Samples& s,
...
bool PreBatch_assigner_rel::is_intersect(Sample &sample, const unordered_map<int,int> &entities,
...
void PreBatch_assigner_rel::assign_for_iteration(int it) {
    ...
    vector<int> sample_count(rel_buckets.size());

    for(int i: indices){
        current_queue.push(i);
    }
    for(int i=0;i<rel_buckets.size();++i){
        sample_count[i]=0;
...
//                real_size[i]=0;
        ...
        for (int i = 0; i < rel_buckets.size(); ++i) {
            if (is_intersect(sample,rel_buckets[i],rel_buckets[i])) {
                if(to_insert<0) {
                    to_insert = i;
                }
                else{
...
//                real_size[to_insert]++;
        ...
        to_insert = -1;
        for (int i = 0;i < rel_buckets.size();++i) {
            value_type new_size=sample_count[i]+real_rel_size[i]*parameter->est_rel_coeff;
...
#endif //DISTRESCAL_PREBATCHASSIGNER_RELONLY_H
