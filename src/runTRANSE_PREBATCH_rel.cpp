...
#include "../alg/TransE_Prebatch_rel.h"
...
int main(int argc, char **argv) {
    ...
    desc.add_options()
            ...
            ("opt", po::value<string>(&(parameter.optimization))->default_value("AdaGrad"),
             "optimization method, i.e., SGD, AdaGrad or AdaDelta")
            ("d", po::value<int>(&(parameter.dimension))->default_value(200), "number of dimensions")
            ("show_loss", po::value<bool>(&(parameter.show_loss))->default_value(0),
             "whether the program shows loss values during training")
            ("lambdaA", po::value<value_type>(&(parameter.lambdaA))->default_value(0),
             "regularization weight for entity")
            ("lambdaR", po::value<value_type>(&(parameter.lambdaR))->default_value(0),
            ...
            ("thre_freq", po::value<value_type>(&(parameter.threshold_freq))->default_value(0.5),
             "threshold for frequent entities")
            ("est_entity_coeff", po::value<value_type>(&(parameter.est_entity_coeff))->default_value(1),
             "coefficient for real_size+sample_size")
            ("est_rel_coeff", po::value<value_type>(&(parameter.est_rel_coeff))->default_value(1),
             "coefficient for real_size+sample_size")
            ...
            ("n_e", po::value<int>(&(parameter.num_of_eval_thread))->default_value(-1),
             "number of threads for evaluation. -1: automatically set")
...
//  data.output_decoder(parameter.output_path);
    ...
    if (parameter.optimization == "sgd") {
        TRANSE_PREBATCH_REL<sgd> rescal_lock(parameter, data);
        rescal_lock.train();
    } else if (parameter.optimization == "adagrad") {
        TRANSE_PREBATCH_REL<adagrad> rescal_lock(parameter, data);
        rescal_lock.train();
    } else if (parameter.optimization == "adadelta") {
        TRANSE_PREBATCH_REL<adadelta> rescal_lock(parameter, data);
        rescal_lock.train();
    } else {
