...
int main(int argc, char **argv) {
    ...
    desc.add_options()
            ...
            ("lambdaA", po::value<value_type>(&(parameter.lambdaA))->default_value(0.1),
             "regularization weight for entity")
            ("lambdaR", po::value<value_type>(&(parameter.lambdaR))->default_value(0.01),
...
//  data.output_decoder(parameter.output_path);
    ...
    if (parameter.optimization == "sgd") {
        RESCAL_NAIVE<sgd> rescal_naive(parameter, data);
        rescal_naive.train();
    } else if (parameter.optimization == "adagrad") {
        RESCAL_NAIVE<adagrad> rescal_naive(parameter, data);
        rescal_naive.train();
    } else if (parameter.optimization == "adadelta") {
        RESCAL_NAIVE<adadelta> rescal_naive(parameter, data);
        rescal_naive.train();
    } else {
