...
//  data.output_decoder(parameter.output_path);
    ...
    if (parameter.optimization == "sgd") {
        RESCAL_NLL<sgd> rescal_lock(parameter, data);
        rescal_lock.train();
    } else if (parameter.optimization == "adagrad") {
        RESCAL_NLL<adagrad> rescal_lock(parameter, data);
        rescal_lock.train();
    } else if (parameter.optimization == "adadelta") {
        RESCAL_NLL<adadelta> rescal_lock(parameter, data);
        rescal_lock.train();
    } else {
