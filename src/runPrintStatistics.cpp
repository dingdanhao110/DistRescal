...
//  data.output_decoder(parameter.output_path);
    ...
    vector<int> rel_statistics(data.num_of_relation);

    for(int idx=0;idx<data.num_of_training_triples;++idx){
        const Triple<int> &true_triple = data.training_triples.at(idx);
        rel_statistics[true_triple.relation]++;
    }

    vector<pair<int,int>> heap_rel_vec(0);
    heap_rel_vec.reserve(rel_statistics.size());
    ...
    for(const auto& pair:heap_rel_vec){
