...
#include "../alg/RESCAL_Prebatch_full.h"
...
//  data.output_decoder(parameter.output_path);
    ...
    if (parameter.optimization == "sgd") {
        RESCAL_PREBATCH_FULL<sgd> rescal_lock(parameter, data);
        rescal_lock.train();
    } else if (parameter.optimization == "adagrad") {
        RESCAL_PREBATCH_FULL<adagrad> rescal_lock(parameter, data);
        rescal_lock.train();
    } else if (parameter.optimization == "adadelta") {
        RESCAL_PREBATCH_FULL<adadelta> rescal_lock(parameter, data);
        rescal_lock.train();
    } else {
